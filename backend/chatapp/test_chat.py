...
class ChatAPITests(APITestCase):
    def setUp(self):
        self.user = MyUser.objects.create_user(email='test@example.com', name='Test User', password='password123')
    ...
    def test_create_chat_message(self):
        session = ChatSession.objects.create(user_id_str='guest_123', user_name='Guest User')
        data = {
            'session': session.id,
            'sender': 'user',
            'text': 'Hello support'
