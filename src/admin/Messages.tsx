...
const AdminMessages = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = chatService.subscribeToMessages(selectedSession.id, (msgs) => {
      setMessages(msgs);
      // Mark as read when messages are received and session is active
  ...
  const handleSend = async (textOverride?: string, attachments?: string[]) => {
    ...
    try {
      await chatService.sendMessage(selectedSession.id, text, "human", undefined, attachments);
    } catch (error) {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        setIsUploading(true);
        try {
          const url = await chatService.uploadFile(audioBlob, path);
    ...
    return (
      <div className="mt-2 space-y-2">
        {attachments.map((url, i) => {
          ...
          const isAudio = url.match(/\.(mp3|wav|webm|ogg|m4a|mp4|aac)/i) || url.includes("audio") || url.includes("voice_note");
          ...
          if (isAudio) {
            return (
              <div key={i} className="bg-muted p-2 rounded-lg flex items-center gap-2">
          ...
          <>
            ...
            <div className="p-6 bg-card border-t border-border shrink-0">
              <div className="max-w-4xl mx-auto flex items-center gap-3">
                ...
                {!isRecording && (
                  <button 
                    className={`p-2 rounded-xl transition-colors ${isRecording ? "text-rose-500 bg-rose-50" : "text-muted-foreground hover:bg-muted hover:text-emerald-500"}`}
                ...
                <button 
                  onClick={() => handleSend()}
                  disabled={(!inputValue.trim() && !isRecording) || !selectedSession || isUploading}
