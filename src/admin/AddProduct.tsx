...
const AddProduct = () => {
  ...
  const handleAddNewBrand = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to add new brand");
    }
  };
  ...
  const handleCreateGlobalBrand = async () => {
    ...
    try {
      ...
      if (targetSlugs.length > 0) {
        ...
        await queryClient.invalidateQueries({ queryKey: ['categories'] });
        const fromCategories = (await categoryService.getAll()).flatMap((c: any) => c.brands || []);
      ...
      console.error('Error creating global brand:', error);
      toast.error(error.message || 'Failed to create global brand');
    }
  };
  ...
  const handleAddNewType = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to add new subcategory");
    }
  };
  ...
  const handleAddBrandToCategory = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to add brand to category");
    }
  };
  ...
  const handleDeleteType = async () => {
    ...
    try {
      ...
      if (categoryData) {
        ...
        const newTypes = currentTypes.filter((t: any) => t.name !== formData.type && t.slug !== formData.type);
  ...
  const handleAddType = async () => {
    ...
    try {
      ...
      const currentTypes = categoryData.subcategories || [];
      if (currentTypes.some((t: any) => t.name === formData.type.trim())) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // === CRITICAL FIX: Resolve Brand ID ===
      let brandId: number | null = null;
      const brandObj = dbBrands.find((b: any) => 
      ...
      const errorMsg = error?.response?.data?.brand?.[0] || 
        ...
        <main className="neo-container py-8 max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                          ...
                          {categories.map((cat) => {
                            ...
                            return (
                              <button
                                key={cat.slug}
                                type="button"
                                onClick={() => handleCategoryChange({ target: { value: cat.name } } as any)}
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (!newCategory || PRESET_ICONS.includes(newCategory)) {
                                      setNewCategory(icon);
                                    }
                                  }}
                ...
                {COLLECTIONS.map(collection => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl border text-xs font-bold uppercase tracking-widest transition-all ${isSelected ? "border-primary bg-primary/5 text-primary ring-1 ring-primary" : "border-border bg-accent/30 text-muted-foreground hover:bg-accent/50"
                        }`}
                    >
                      {collection}
                    </button>
            ...
            <section id="specs" className="bg-card border border-border rounded-2xl p-6 shadow-sm gsap-reveal">
              <div className="flex items-center gap-2 mb-6">
                <Package size={18} className="text-primary" />
                <h2 className="text-sm font-bold uppercase tracking-widest">Details & Specs</h2>
              </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-accent/30 border border-border rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none resize-none"
                  />
                </div>

                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Key Specifications</label>
                    <button
                      type="button"
