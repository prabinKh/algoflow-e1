...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Clear cart in store and backend
      cartStore.clear();
      await customerService.clearCart(user.id);

      toast.success("Order placed successfully!");
      // Ensure id is always a string (Django returns integer PKs)
      navigate("/order-success", { state: { order: { ...orderData, id: String(orderDoc.id ?? orderDoc.orderId ?? orderDoc.order_id ?? "") } } });
    } catch (error) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2 md:col-span-2">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <MapPin size={12} /> Capture Location
                    </button>
                  </div>
                  ...
                  <div key={item.product.id} className="flex gap-3 sm:gap-4 p-3 sm:p-4 bg-card border border-border rounded-xl shadow-[var(--shadow-sm)] gsap-reveal">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <span className="font-bold text-sm">Rs. {((item.product.price || 0) * (item.quantity || 0)).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-card border border-border rounded-2xl p-6 h-fit sticky top-24 shadow-[var(--shadow-md)] gsap-reveal-sidebar">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold">Rs. {(total || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-display font-bold text-primary">Rs. {(total || 0).toLocaleString()}</span>
                </div>
              </div>
